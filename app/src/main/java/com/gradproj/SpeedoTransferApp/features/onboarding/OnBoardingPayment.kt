...
@Composable
fun OnBoardingPayment(navController: NavController) {
    // Root container
    Column(
        modifier = Modifier
            .fillMaxSize()
        ...
        CustomButton("Next", onClick = { navController.navigate(Screen.Signin.route) }, buttonType = "Filled")
        Spacer(modifier = Modifier.height(16.dp))
    }
}
...
@Composable
private fun OnBoardingPaymentprev() {
    OnBoardingPayment(rememberNavController())
}